...
function ChatInterface({
  ...
  const [superchatContractAddress, setSuperchatContractAddress] = useState(
    '0xE85b157E7685Ce6Bc35fd33c1dfb7E887E7470AF'
  )
  ...
  useEffect(() => {}, [messages, isLocked])

  const setCurrentAccount = async () => {
    web3 = new Web3(window.ethereum)
    SuperChatContract = new web3.eth.Contract(
      SuperchatABI,
      superchatContractAddress
    )
  }
  ...
  const sendMessage = async () => {
    let newMessage = await pubsub.publish(pubsubTopic, {
      _id: uuidv4(),
      message: message,
  ...
  const sendSuperChat = async () => {
    if (parseFloat(superChatValue) > 0) {
      ...
      SuperChatContract.methods
        .superChat(creatorAccount, weiAmount, message, uuidv4())
  ...
  return (
    ...
    >
      {isConnectingToDatabase ? (
        <Tag justifySelf="center" alignSelf="center">
          {loadingText}
        </Tag>
      ) : (
        <>
                  ...
                  : messages.map((message) => {
                      return message.isSuperChat ? (
                        <VStack
                          ...
                          key={message._id}
                          borderRadius={5}
                          background={seedColor(message.sender).toHex()}
                        >
                          <HStack color="white" width="100%" px={5} py={2}>
                            <Avatar
                              ...
                              size="md"
                              bg="transparent"
                              src={svgAvatarGenerator(message.sender, {
                                dataUri: true,
                              })}
                            />
                            <VStack alignItems="flex-start" width="100%">
                              <Text>
                                {message.sender.substr(0, 6)}...
                                {message.sender.substr(-6)}
                              </Text>
                        ...
                        <Box key={message._id} maxWidth="18vw">
                          <Text
                            isTruncated
                            color={seedColor(message.sender).toHex()}
                            as="i"
                          >
                            {message.sender.substr(0, 3)}...
                            {message.sender.substr(-3)}
                          </Text>
              ...
              {isLocked ? (
                ...
                >
                  <Tag backgroundColor="rgba(230,1,122,0.08)" color="#E6017A">
                    Please Start Streaming To Chat
                  </Tag>
                </VStack>
              ) : (
                <VStack justifySelf="flex-end" width="100%">
                  ...
                  <HStack width="100%">
                    <Input
                      ...
                      value={superChatValue}
                      focusBorderColor="pink.400"
                      onChange={handleSuperChatValue}
                    />
                    <Button
                      isLoading={isSuperChatting}
                      onClick={sendSuperChat}
                      colorScheme="red"
                    >
                      SuperChat
                    </Button>
                    <Button onClick={sendMessage} colorScheme="facebook">
                      Send
                    </Button>
                  </HStack>
                </VStack>
