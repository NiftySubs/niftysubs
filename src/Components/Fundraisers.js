...
const FundraisingContract = new web3.eth.Contract(
  FundraisingContractABI,
  '0x608DA975Dd743Bde9ab6329258E4AD3619A533EF'
)
...
function Fundraisers({ currentAccount }) {
  ...
  const getFundraises = useCallback(async () => {
    ...
    const fundraisesId = await FundraisingContract.methods
      .getFundraises(currentAccount)
      .call()
    let fundraises = []
    for (const id of fundraisesId) {
      const fundraise = await FundraisingContract.methods
        .getFundraise(id)
        .call()
  ...
  const init = async () => {
    ...
    FundraisingContract.once(
      'FundraiseCreated',
      { filter: { fundraiser: currentAccount }, fromBlock: getBlock },
              ...
              fundraises.map((fundraise) => {
                return (
                  <Tr key={fundraise['id']}>
                    ...
                    <Td>
                      <HStack>
                        <Text>{fundraise['fundraiseAgenda']}</Text>
                        <FaExternalLinkAlt />
                      </HStack>
                    </Td>
                    <Td>
                      <Text>{`${fundraise['beneficiary'].substr(
                        0,
                        5
                      )}...${fundraise['beneficiary'].substr(-5)}`}</Text>
                    </Td>
                    <Td>
                      {`${web3.utils.fromWei(
                        fundraise['amountFunded'],
                        'ether'
                      )} ETH`}
                    </Td>
                    <Td>
                      {`${web3.utils.fromWei(
                        fundraise['fundraiseGoal'],
                        'ether'
                      )} ETH`}
                    </Td>
                    <Td>
                      <Tag>{`${new Date(
                        parseInt(fundraise['fundraiseDeadline']) * 1000
                      ).toDateString()}`}</Tag>
                    </Td>
                    <Td>
                      <Button onClick={() => claimFunds(fundraise['id'])}>
                        Claim
                      </Button>
                    </Td>
                  </Tr>
            ...
            <VStack>
              <FormControl id="agenda" isRequired>
                ...
                <Input
                  ...
                  value={fundraiseAgenda}
                  onChange={(e) => handleInput(e, setFundraiseAgenda)}
                />
              </FormControl>
              <FormControl id="beneficiary" isRequired>
                ...
                <Input
                  focusBorderColor="pink.500"
                  value={fundraiseBeneficiary}
                  onChange={(e) => handleInput(e, setFundraiseBeneficiary)}
                />
              </FormControl>
              <FormControl id="goal" isRequired>
                ...
                <Input
                  focusBorderColor="pink.500"
                  value={fundraiseGoal}
                  onChange={(e) => handleInput(e, setFundraiseGoal)}
                />
              </FormControl>
              <FormControl id="agenda" isRequired>
                ...
                <Input
                  type="date"
                  focusBorderColor="pink.500"
                  value={fundraiseDeadline}
                  onChange={(e) => handleInput(e, setFundraiseDeadline)}
                />
              </FormControl>
            </VStack>
          </ModalBody>
          <ModalFooter>
            <Button onClick={() => createFundraise(onClose)}>
              Create Fundraise
            </Button>
          </ModalFooter>
        </ModalContent>
